// OTOMATİK ÜRETİLDİ — elle düzenleme! Kaynak: albrus-firma/main.js
...
function cekHareketGeriAl(cs) {
  ...
  `, [kasa_id]);
  H["kasa:rapor"] = (_, { bas, bit, kasa_id }) => {
  const where = [];
  const params = [];
  ...
  `, [hesap_id]);
  H["banka:rapor"] = (_, { bas, bit, hesap_id }) => {
  const where = [];
  const params = [];
  ...
  H["fatura:teklif:donustur"] = (_, id, hedef) => {
  const f = getOne('SELECT * FROM faturalar WHERE id = ?', [id]);
  if (!f) throw new Error('Belge bulunamadı.');
  ...
  H["puantaj:mesai:guncelle"] = (_, personel_id, yil, ay, gun, mesai_saat) => {
  ...
  H["personel:ekstre"] = (_, personel_id) => {
  ...
  H["ortak:hareketler:getir"] = (_, filter = {}) => {
  ...
  H["hakedis:poz:sil"] = (_, id) => {
  run('DELETE FROM hakedis_pozlar WHERE id = ?', [id]);
  saveDb();
  return true;
};
  H["hakedis:poz:tumunu:sil"] = (_, proje_id) => {
  ...
  H["hakedis:boq:import"] = (_, { proje_id, satirlar, temizle }) => {
  ...
  H["yesil:kaydet"] = (_, hakedis_id, satirlar) => {
  for (const s of (satirlar || [])) {
  ...
  H["icmal:getir"] = (_, hakedis_id, mod) => hesaplaIcmal(hakedis_id, mod);
  H["ilave:getir"] = (_, hakedis_id) => {
  const h = getOne('SELECT * FROM hakedisler WHERE id = ?', [hakedis_id]);
  if (!h) return null;
  ...
  H["ilave:kaydet"] = (_, hakedis_id, satirlar) => {
  ...
  H["kesinti:kaydet"] = (_, hakedis_id, satirlar) => {
  ...
  H["arkakapak:getir"] = (_, hakedis_id) => {
  const h = getOne('SELECT * FROM hakedisler WHERE id = ?', [hakedis_id]);
  if (!h) return null;
  ...
  H["arkakapak1:getir"] = (_, hakedis_id) => {
  const h = getOne('SELECT * FROM hakedisler WHERE id = ?', [hakedis_id]);
  if (!h) return null;
  const proje = getOne('SELECT * FROM projeler WHERE id = ?', [h.proje_id]);
  ...
  H["progress:kaydet"] = (_, hakedis_id, data) => {
  data = data || {};
